...
class PasswordField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Widget _buildBar(int n, String pass) {
      final int level = _calcStore(pass);
      return Expanded(
        child: Container(
    ...
    return FormField<String>(
      ...
      onSaved: onSaved,
      validator: (text) {
        if (text.isEmpty || _calcStore(text) < 2)
          ...
          children: <Widget>[
            ...
            Padding(
              ...
              child: state.value.isNotEmpty || state.hasError ? Text(
                state.value.isNotEmpty ? _getText(_calcStore(state.value)) : state.errorText,
                textAlign: TextAlign.start,
                style: TextStyle(
                  color: _getColor(_calcStore(state.value)),
                  fontSize: 12
                ),
              ) : Container(),
            )
